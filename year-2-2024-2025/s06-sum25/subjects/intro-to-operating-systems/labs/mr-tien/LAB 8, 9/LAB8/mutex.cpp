...
int main() {
    ...
    // Tạo tuyến thứ nhất 
    res = pthread_create(&p_thread1, NULL, do_thread1, NULL); // sửa tên biến và hàm
    if (res != 0) {
    ...
    // Tạo tuyến thứ hai 
    res = pthread_create(&p_thread2, NULL, do_thread2, NULL); // sửa tên biến và hàm
    if (res != 0) {
    ...
    pthread_mutex_destroy(&a_mutex); // giải phóng mutex
    return 0; 
}